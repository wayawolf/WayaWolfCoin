...
{
    return GetBoolArg("-testnet", false) ? 24122 : 14122;
}
...
Value stop(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "stop\n"
            "Stop RainbowLite server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "RainbowLite server stopping";
}
...
string HTTPPost(const string& strMsg, const map<string,string>& mapRequestHeaders)
{
    ostringstream s;
    s << "POST / HTTP/1.1\r\n"
      << "User-Agent: RainbowLite-json-rpc/" << FormatFullVersion() << "\r\n"
      << "Host: 127.0.0.1\r\n"
...
public:
    ...
    {
        ip::tcp::resolver resolver((asio::io_service&)(stream.get_executor().context()));
...
void ThreadRPCServer(void* parg)
{
    ...
    RenameThread("RainbowLite-rpclist");
...
{
    // Accept connection
    boost::asio::io_service& io_service = (boost::asio::io_service&)(acceptor->get_executor().context());
...
void ThreadRPCServer2(void* parg)
{
        ...
        (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"]))
    {
        ...
        string strWhatAmI = "To use RainbowLite";
...
void ThreadRPCServer3(void* parg)
{
    ...
    RenameThread("RainbowLite-rpchand");
